...
function MetricCard({
  ...
  const variantBorder =
    variant === 'positive'
      ? 'border-positive/30 glow-primary'
      : variant === 'negative'
        ? 'border-negative/30 glow-negative'
        : variant === 'warning'
  ...
  return (
    <div
      className={`card-surface p-5 flex flex-col gap-3 ${variantBorder} ${span === 2 ? 'col-span-2' : ''} hover:border-primary/20 transition-colors duration-200`}
    >
      ...
      <div className="flex items-end justify-between gap-2">
        <div>
          <p
            className={`text-2xl font-bold font-tabular leading-none ${valueColor} ${mono ? 'font-mono' : ''}`}
          >
            {value}
          </p>
          {subValue && (
        ...
        {change && (
          <div
            className={`flex items-center gap-1 text-xs font-semibold font-tabular px-2 py-1 rounded-full ${changePositive ? 'bg-positive-subtle text-positive' : 'bg-negative-subtle text-negative'}`}
          >
...
export default function LiveMetricCards() {
  ...
  const positionCount =
    realtimeData?.positions?.filter((pos) => parseFloat(pos.size) !== 0).length || 0;
  const totalPositionValue =
    realtimeData?.positions?.reduce((sum, pos) => {
